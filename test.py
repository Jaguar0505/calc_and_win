...
class Penguin(Bird):
    ...
    def describe(self, full=False):
        if full is False:
            return f'Размер птицы {self.name} — {self.size}.'
        if full is True:
            return f'Размер пингвина {self.name} из рода genus — {self.size}. Интересный факт: однажды группа геологов-разведчиков похитила пингвинье яйцо, и их принялась преследовать вся стая, не пытаясь, впрочем, при этом нападать. Посовещавшись, похитители вернули птицам яйцо, и те отстали.'
